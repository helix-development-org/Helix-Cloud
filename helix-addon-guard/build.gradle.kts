 ...
 * gate) plus the IGuard Paper plugin (built from the sibling
 * `helix-addon-guard-paper` module) and the bundled packetevents
...
val packageHxa by tasks.registering(Zip::class) {
    ...
    from(project(":helix-addon-guard-paper").tasks.named<Jar>("jar").flatMap { it.archiveFile }) {
        rename { "paper.jar" }
    }
